  ...
  reloadModule(indexPath, () => {
    if (options.virtualRequires) {
      for (const [modPath, exportsObj] of Object.entries(options.virtualRequires)) {
        if (exportsObj === null) {
          jest.doMock(modPath, () => { throw new Error(`Failed to load ${path.basename(modPath)}`); }, { virtual: true });
        } else {
          stubModule(modPath, exportsObj);
        }
      }
    }
...
test('should index cache limits keep only the bot user over limit', () => {
  ...
  try {
    ...
    expect(memberKeep({ id: 'bot-123' })).toBe(true);
    expect(memberKeep({ id: 'other' })).toBe(false);
    expect(userKeep({ id: 'bot-123' })).toBe(true);
    expect(userKeep({ id: 'other' })).toBe(false);
  } finally {
    restore();
  }
});

test('should index wires command and signal handlers (coverage merged)', async () => {
...
test('should index ignores non-guild interactions when allowlist is configured', async () => {
...
test('should index blocks interactions outside allowed guilds', async () => {
  const { client, restore } = loadIndexHarness({ allowedGuildIds: new Set(['allowed-guild']) });
  try {
    client.commands.set('ok', { execute: async () => {} });
    let replied = false;
...
test('should index records command reply failures using httpStatus only', async () => {
  ...
  try {
    ...
    expect(commandReplyMetrics).toBe(1);
  } finally {
    restore();
  }
});
...
test('should index registers process handlers and handles shutdown failures', async () => {
  delete require.cache[instrumentPath];
  stubModule(instrumentPath, {
    Sentry: { isEnabled: () => false },
    captureError: () => {},
...
test('should index logs command and event load failures and event handler errors', async () => {
  ...
  try {
    ...
    expect(onceHandlers.length).toBe(1);
    expect(repeatHandlers.length).toBe(1);
